...
function EnviarModal({ lote, workspaceId, onClose, onSent }) {
  ...
  function handleWA() {
    ...
    const msg = encodeURIComponent(
      `Olá! Segue o link para aprovação do lote "${lote.cliente}":\n${link}\n\nPor favor, acesse e confirme o De Acordo.`
    )
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-secondary)', borderRadius: 18, width: '100%', maxWidth: 480, border: '1px solid var(--border)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '18px 22px' }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
            <button onClick={handleWA} disabled={!link || !telefone.replace(/\D/g, '').length}
              style={{ flex: 1, padding: '10px', borderRadius: 9, background: '#25D366', border: 'none', cursor: 'pointer', color: '#fff', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, opacity: (!link || !telefone.replace(/\D/g, '').length) ? 0.45 : 1 }}>

...
export default function LiderEpi() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <Header title="?? Controle de EPIs / EPCs" subtitle="Solicita��es, cat�logo individual e coletivo" />
...
function SecaoSolicitacoes({ workspaceId }) {
  ...
  async function confirmarReprovacao() {
    if (!motivoReprov.trim()) { toast.error('Informe o motivo da reprova��o'); return }
    setSaving(true)
    const { data: { user } } = await supabase.auth.getUser()
    const { error } = await supabase.from('lider_solicitacoes_epi').update({
      status:            'reprovado',
    ...
    setSaving(false)
    if (error) { toast.error(error.message); return }
    toast.success('Solicita��o reprovada')
    ...
    <div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--text-secondary)' }}>
          ...
          <p style={{ fontSize: 13 }}>
            {buscaColaborador.trim() ? `Nenhum colaborador encontrado para "${buscaColaborador.trim()}"` : 'Nenhuma solicita��o de EPI encontrada'}
          </p>
        </div>
      ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-muted)' }}>
                  {['Data', 'Colaborador', 'EPI', 'Qtd', 'Motivo', 'Frente / Equipe', 'Foto', 'Status', 'A��es'].map(h => (
      ...
      {/* -- Modal Reprovar ------------------------------------------------ */}
      {modalReprovar && (
        <Modal title="Reprovar solicita��o" onClose={() => setModalReprovar(null)} maxWidth={440}>
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12 }}>Informe o motivo da reprova��o. O l�der ser� notificado.</p>
          <label style={lbl}>Motivo *</label>
          <textarea
            value={motivoReprov}
            onChange={e => setMotivoReprov(e.target.value)}
            rows={3}
            placeholder="Ex: EPI dispon�vel no almoxarifado..."
            className="input"
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 20 }}>
            ...
            >{saving ? 'Salvando...' : 'Confirmar reprova��o'}</button>
          </div>
        </Modal>
      )}
    </div>
...
// -------------------------------------------------------------------------------
// SE��O � CAT�LOGO DE EPIs (Individual � por colaborador)
// -------------------------------------------------------------------------------
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          <span style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-primary)' }}>Cat�logo de EPIs</span>
        ...
        <Modal title={modal === 'novo' ? 'Novo EPI' : `Editar: ${modal.nome}`} onClose={() => setModal(null)} maxWidth={400}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              ...
              <input className="input" value={nome} onChange={e => setNome(e.target.value)} placeholder="Ex: Capacete de seguran�a" />
            </div>
            <div>
              <label style={lbl}>CA (Certificado de Aprova��o)</label>
...
// -------------------------------------------------------------------------------
// SE��O � CAT�LOGO DE EPCs (Coletivo � por m�dulo / frente de trabalho)
// -------------------------------------------------------------------------------
function SecaoCatalogoEPC({ workspaceId }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          <span style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-primary)' }}>Cat�logo de EPCs</span>
          <span style={{ fontSize: 11, color: 'var(--text-secondary)', marginLeft: 8 }}>Equipamento de Prote��o Coletiva � por m�dulo</span>
          <span style={{ fontSize: 12, color: 'var(--text-secondary)', marginLeft: 10 }}>{ativos} ativo{ativos !== 1 ? 's' : ''}</span>
        </div>
        <button onClick={abrirNovo} className="btn-primary" style={{ fontSize: 13, padding: '7px 14px', display: 'flex', alignItems: 'center', gap: 5 }}>
      ...
      {modal && (
        <Modal title={modal === 'novo' ? 'Novo EPC' : `Editar: ${modal.nome}`} onClose={() => setModal(null)} maxWidth={420}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <label style={lbl}>Nome *</label>
              <input className="input" value={nome} onChange={e => setNome(e.target.value)} placeholder="Ex: Sinaliza��o de seguran�a" />
            </div>
            <div>
              <label style={lbl}>CA (Certificado de Aprova��o)</label>
              <input className="input" value={ca} onChange={e => setCa(e.target.value)} placeholder="Ex: 12345" />
            ...
            <div>
              <label style={lbl}>M�dulo / Frente de trabalho</label>
              {frentes.length > 0 ? (
                <select className="input" value={frente} onChange={e => setFrente(e.target.value)}>
                  <option value="">� Sem m�dulo espec�fico �</option>
